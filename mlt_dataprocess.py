...
def Load_ChannelCharacteristicsData(args):
    datapath = args.datapath ; test_ratio =args.test_ratio ; seed = args.seed
    ...
    mask_value = 1e-5
    # mask_value = 1
    mask_train = torch.zeros(channel_data_train.shape[0],1,200,200)
    mask_train = mask_train + channel_data_train[:,[6],:,:]
    mask_train[mask_train<1]=-1; mask_train[mask_train>0]=-mask_value; mask_train=-1*mask_train

    mask_test = torch.zeros(channel_data_test.shape[0],1,200,200)
    mask_test = mask_test + channel_data_test[:,[6],:,:]
    mask_test[mask_test<1]=-1; mask_test[mask_test>0]=-mask_value; mask_test=-1*mask_test

    for i in np.arange(0, 199, args.scale):
...
def INterplate(channel_data, scale, MODE):
    Output_data = F.interpolate(channel_data, scale_factor = 1 / scale ,mode= MODE, align_corners=True)
    Output_data = F.interpolate(Output_data, scale_factor = scale, mode = MODE, align_corners=True)

    for i in np.arange(0, 199, scale):
        for j in np.arange(0, 199, scale):
            Output_data[:, :, i, j] = channel_data[:, :, i, j]
